...
/**
 * Discover available models from DeepSeek Web.
 * @param {string} token
 * @returns {Promise<Array<{id: string, thinkingEnabled: boolean, searchEnabled: boolean}>>}
 */
export async function discoverDeepSeekModels(token) {
    const params = new URLSearchParams({ did: crypto.randomUUID(), scope: 'model' });
    const resp = await fetch(`${BASE_URL}/api/v0/client/settings?${params}`, {
        headers: buildAuthHeaders(token),
    });

    if (!resp.ok) {
        throw new Error(`Model discovery failed: HTTP ${resp.status}`);
    }

    const data = await resp.json();
    const modelConfigs = data?.data?.biz_data?.settings?.model_configs?.value || [];

    const models = [];
    for (const mc of modelConfigs) {
        if (!mc.model_type || !mc.enabled) continue;
        const hasThink = mc.think_feature != null;
        const hasSearch = mc.search_feature != null;

        models.push({
            id: mc.model_type === 'default' ? 'deepseek-default' : `deepseek-${mc.model_type}`,
...
export { STORAGE_KEYS as DEEPSEEK_WEB_STORAGE_KEYS };
