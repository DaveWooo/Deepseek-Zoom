...
| **DeepSeek Web (Free)** | `deepseek_web.js`      | DeepSeek (Unified: Fast + R1 Thinking + Multimodal) | **No API key**; reuses the free `chat.deepseek.com` session; unified model with reasoning display and web search; direct image upload (PoW + fork) | DeepSeek account login (phone/email + password in Settings) |
